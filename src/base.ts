...
export namespace Convert {
  ...
  export function str2bin(str: string): Uint8Array {
  	str = str.replace(/\r\n/g, '\n');
  	const bin = new Uint8Array(str.length * 3); let p = 0;
  ...
  export function base642bin(base64: string): Uint8Array | undefined {
  	...
  	if (typeof atob !== 'undefined') {
  		...
  		let p = 0; const bin = new Uint8Array(strlen);
  		for (let i = 0, len = base64.length; i < len;) {
  			const sextet_a = base64.charAt(i) === '=' || base64.charCodeAt(i) > 'z'.charCodeAt(0) ? 0 : decodingTable[base64.charCodeAt(i)]; i++;
  			const sextet_b = base64.charAt(i) === '=' || base64.charCodeAt(i) > 'z'.charCodeAt(0) ? 0 : decodingTable[base64.charCodeAt(i)]; i++;
  			const sextet_c = base64.charAt(i) === '=' || base64.charCodeAt(i) > 'z'.charCodeAt(0) ? 0 : decodingTable[base64.charCodeAt(i)]; i++;
  			const sextet_d = base64.charAt(i) === '=' || base64.charCodeAt(i) > 'z'.charCodeAt(0) ? 0 : decodingTable[base64.charCodeAt(i)]; i++;
   ...
   */
  export function bin2hex(bin: Uint8Array, uppercase = false): string {
  ...
  export function bin2str(bin: Uint8Array): string {
  	let str = '', i = 0, c, c2, c3; const len = bin.length;
   ...
   */
  export function bin2base64(bin: Uint8Array, url = false): string {
  	if (typeof btoa !== 'undefined') {
  		...
  		for (let i = 0, len = bin.length; i < len;) {
  			const octet_a = i < bin.length ? bin[i] : 0; i++;
  			const octet_b = i < bin.length ? bin[i] : 0; i++;
  			const octet_c = i < bin.length ? bin[i] : 0; i++;
