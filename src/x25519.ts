...
//   ▀██████▀             ▀████▄▄▄████▀       for its {ab,mis,}use.
//                           ▀█████▀▀
// curve25519 (scalarmult and ed25519) implementation
//
// inspired by:
...
//
// test vectors:
// @see https://tools.ietf.org/html/draft-josefsson-eddsa-ed25519-03
//
...
export class Curve25519 {
   ...
   */
	...
	pack25519(o: Uint8Array, n: Int32Array): void {
		let i; const m = this.gf(), t = this.gf();
		for (i = 0; i < 16; i++) {
			t[i] = n[i];
		}
		this.car25519(t);
   ...
   */
	private crypto_scalarmult(q: Uint8Array, s: Uint8Array, p: Uint8Array): void {
		const x = new Int32Array(80); let r, i;
		const a = this.gf(), b = this.gf(), c = this.gf(),
			d = this.gf(), e = this.gf(), f = this.gf();
   ...
   */
	selftest(): boolean {
		...
		for (let i = 0, len = key.length; i < len; i++) {
			...
			if (!keys || !Util.compare(keys.pk, pk)) return false;
		}
...
export class Ed25519 implements Signature {
   ...
   */
	...
	private pack(r: Uint8Array, p: Int32Array[]): void {
	...
	private reduce(r: Uint8Array): void {
		let i; const x = new Uint32Array(64);
		for (i = 0; i < 64; i++) {
			x[i] = r[i];
		}
		this.modL(r, x);
	...
	private scalarmult(p: Int32Array[], q: Int32Array[], s: Uint8Array): void {
		const CURVE = this.curve;
	...
	private scalarbase(p: Int32Array[], s: Uint8Array): void {
		const CURVE = this.curve;
   ...
   */
	sign(msg: Uint8Array, sk: Uint8Array, pk: Uint8Array): Uint8Array | undefined {
		...
		const x = new Uint32Array(64); let i, j;
   ...
   */
	selftest(): boolean {
		...
		for (let i = 0; i < v.length; i++) {
			const sk = Convert.hex2bin(v[i].sk),
			...
			if (!sig || !Util.compare(sig, s)) return false;
