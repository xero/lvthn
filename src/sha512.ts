...
export class SHA512 implements Hash {
   ...
   */
	private transform(): void {
		const h = this.H;
		const h0h = h[0], h1h = h[2], h2h = h[4], h3h = h[6],
			h4h = h[8], h5h = h[10], h6h = h[12], h7h = h[14];
		let h0l = h[1],  h1l = h[3],  h2l = h[5],  h3l = h[7],
			h4l = h[9],  h5l = h[11], h6l = h[13], h7l = h[15];
		...
		let i; const w = new Uint32Array(160);
		for (i = 0; i < 32; i++) {
   ...
   */
	digest(msg?: Uint8Array): Uint8Array {
		...
		// append '1'
		const b = this.buffer; let idx = this.bufferIndex;
		b[idx++] = 0x80;
		...
		let i; const hash = new Uint8Array(64);
		for (i = 0; i < 16; i++) {
