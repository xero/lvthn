 ...
 */
export function _chacha20Block(key, counter, nonce12) {
 ...
 */
export function _hchacha20(key, nonce16) {
    const s = new Uint32Array(16);
    s[0] = 0x61707865;
    s[1] = 0x3320646e;
    s[2] = 0x79622d32;
    ...
    for (let i = 0; i < 4; i++)
        u32to8le(out, i * 4, s[i]);
    for (let i = 0; i < 4; i++)
        u32to8le(out, 16 + i * 4, s[12 + i]);
    return out;
}
...
function chacha20Crypt(key, counter, nonce12, input) {
    ...
    for (let pos = 0, blk = counter; pos < input.length; pos += 64, blk++) {
        ...
        for (let i = 0; i < end; i++)
            out[pos + i] = input[pos + i] ^ ks[i];
    }
    return out;
}
// Read little-endian bytes as a BigInt.
function leToNum(bytes) {
    let n = 0n;
 ...
 * @returns       16-byte tag
 */
export function _poly1305Mac(key, message) {
 ...
 */
export function _poly1305KeyGen(key, nonce12) {
...
function buildAeadMacInput(aad, ct) {
    const p16 = (n) => (n % 16 === 0 ? 0 : 16 - (n % 16));
    const buf = new Uint8Array(aad.length + p16(aad.length) + ct.length + p16(ct.length) + 16);
    let pos = 0;
    buf.set(aad, pos);
    pos += aad.length + p16(aad.length);
    buf.set(ct, pos);
    pos += ct.length + p16(ct.length);
...
export class ChaCha20Poly1305 {
   ...
   */
    encrypt(key, nonce, plaintext, aad = new Uint8Array(0)) {
        ...
        if (nonce.length !== 12)
            throw new Error('ChaCha20Poly1305: nonce must be 12 bytes');
   ...
   */
    decrypt(key, nonce, ciphertext, tag, aad = new Uint8Array(0)) {
        ...
        if (nonce.length !== 12)
            throw new Error('ChaCha20Poly1305: nonce must be 12 bytes');
        if (tag.length !== 16)
            throw new Error('ChaCha20Poly1305: tag must be 16 bytes');
        const poly1305Key = _poly1305KeyGen(key, nonce);
...
export class XChaCha20Poly1305 {
   ...
   */
    encrypt(key, nonce, plaintext, aad = new Uint8Array(0)) {
        ...
        if (nonce.length !== 24)
            throw new Error('XChaCha20Poly1305: nonce must be 24 bytes');
   ...
   */
    decrypt(key, nonce, ciphertext, tag, aad = new Uint8Array(0)) {
        ...
        if (nonce.length !== 24)
            throw new Error('XChaCha20Poly1305: nonce must be 24 bytes');
        if (tag.length !== 16)
            throw new Error('XChaCha20Poly1305: tag must be 16 bytes');
        const subkey = _hchacha20(key, nonce.subarray(0, 16));
